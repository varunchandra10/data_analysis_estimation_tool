...
from services.archive.engine import (
...
from services.versioning.engine import (
    DEFAULT_PROJECT_ID,
    create_project,
    create_version,
    compare_versions,
...
from services.reporting.engine import generate_report
from services.archive.engine import archive_old_versions as archive_old_versions_service
...
async def api_get_manifest(version_name: str, project_id: str = DEFAULT_PROJECT_ID, dataset_name: str | None = None):
    try:
...
def _bg_generate_report(
    ...
    """Background worker: generates PDF report and registers it in the DB.
    Runs off the FastAPI event loop — all blocking I/O is safe here.
    """
    ...
    filename = f'report_{version_name}_{timestamp}.pdf'
    target_path = REPORTS_DIR / filename
    safe_report_path = None

    try:
        report_path = generate_report(version_name, project_id, dataset_name, output_path=str(target_path))
        safe_report_path = resolve_safe_path(report_path)
        checksum = generate_sha256(safe_report_path)

        db = SessionLocal()
        try:
            resolved_ds_name = resolve_dataset_name(dataset_name or version_name)
            ds = db.query(Dataset).filter(Dataset.dataset_name == resolved_ds_name).first()
            dataset_id = ds.id if ds else 1

            ver = db.query(Version).filter(Version.version_name == version_name).first()
            ...
            db.rollback()
            # Clean up partially-written PDF on DB failure
            if safe_report_path and safe_report_path.exists():
                try:
                    safe_report_path.unlink()
                except Exception:
                    pass
            logger.error(f'DB sync failed during background report generation: {db_exc}', exc_info=True)
            update_task_status(task_id, 'failed', error=f'Database sync error: {db_exc}')
            return
        finally:
            db.close()
        ...
        logger.info(f'Background report generation completed: {filename} (task={task_id})')

    except Exception as exc:
        logger.error(f'Background report generation failed (task={task_id}): {exc}', exc_info=True)
        # Clean up partial PDF artifact
        if safe_report_path and safe_report_path.exists():
            try:
                safe_report_path.unlink()
            except Exception:
...
async def api_generate_report(
    ...
    """Enqueue a PDF report generation job and return immediately.

    P2 FIX: PDF generation (CSV load + matplotlib + reportlab) is moved to a
    BackgroundTask. The endpoint responds with 202 + task_id in <1ms.
    Poll GET /api/report/status/{task_id} to check progress.
    """
    try:
        # Ownership check is synchronous by design — stays on the request thread
        if getattr(payload, 'dataset_name', None):
            _ensure_owner(payload.dataset_name, current_user)

        from services.task_manager import create_task
        task = create_task('report_generation', {
            'version_name': payload.version_name,
            'dataset_name': payload.dataset_name,
        ...
        return JSONResponse(
            ...
            content={
                ...
                'data': {
                    'status': 'queued',
                    'task_id': task_id,
                    'message': 'Poll /api/report/status/{task_id} to track progress.',
                },
                'task_id': task_id,
            },
        )
...
async def api_report_task_status(task_id: str):
    """Poll the status of a background report generation task.

    Returns:
        status: queued | processing | completed | failed
        result.download_url: populated when status == completed
    ...
    return success_response('Task status retrieved.', data=task, task_id=task_id, status=task.get('status'))
...
async def api_reconcile_storage():
    try:
        from services.reconciliation_service import run_reconciliation_audit
        audit_report = run_reconciliation_audit()
        return success_response("Storage reconciliation completed.", data=audit_report, audit=audit_report)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
